...
void static writecommand(uint8_t c) {
  ...
  */
  ...
  ti_lib_gpio_write_dio(BOARD_IOID_DIO1,0);
  board_spi_write(a, 1);
}
...
void static writedata(uint8_t c) {
  ...
  */
  ...
  char a[2]; a[0] = c; a[1] = 0;
  ti_lib_gpio_write_dio(BOARD_IOID_DIO1,1);
  board_spi_write(a, 1);
}
...
void static commonInit(const uint8_t *cmdList) {
  ...
  */
  ...
  //config as output
  ti_lib_ioc_pin_type_gpio_output(BOARD_IOID_DIO1);
  ti_lib_ioc_pin_type_gpio_output(BOARD_IOID_DIO12);
  ti_lib_ioc_pin_type_gpio_output(BOARD_IOID_DIO15);

  //TFT_CS = TFT_CS_LOW;
  ti_lib_gpio_write_dio(BOARD_IOID_DIO12,0);
  //RESET = RESET_HIGH;
  ti_lib_gpio_write_dio(BOARD_IOID_DIO15,1);
  Delay1ms(500);
  //RESET = RESET_LOW;
  ti_lib_gpio_write_dio(BOARD_IOID_DIO15,0);
  Delay1ms(500);
  //RESET = RESET_HIGH;
  ti_lib_gpio_write_dio(BOARD_IOID_DIO15,1);
  Delay1ms(500);
...
//  SSI0_CPSR_R = (SSI0_CPSR_R&~SSI_CPSR_CPSDVSR_M)+16;
  ...
  */
  ...
  board_spi_open(8000000, IOID_21);
